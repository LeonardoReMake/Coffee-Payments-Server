...
- **Celery 5.3.4** — система очередей задач для фоновой обработки
- **Redis 5.0.1** — брокер сообщений для Celery и кэширование
- **Hypothesis 6.92.1** — библиотека для property-based тестирования
...
- `payment_status_service.py` — централизованная логика проверки и обработки статусов платежей с поддержкой fast/slow track
- `yookassa_receipt_service.py` — создание и отправка чеков через ЮKassa
...
/v1/order-status-page
    ...
    │ - Для status=created: кнопка      │
    │   "Перейти к оплате"              │
    │ - Для других статусов: polling    │
    │   каждую секунду                  │
...
[Пользователь оплачивает]
    ...
    /v1/order-status-page?order_id=...
        ↓
    Unified Order Status Page
    (обновляет статус через polling)
        ↑
        │ (параллельно)
        ↓
...
### Система фоновой проверки платежей

Система включает фоновую проверку статуса платежей для обеспечения надежности обработки платежей даже при сбоях webhook'ов.

#### Архитектура фоновой проверки

- **Celery Beat Scheduler** — запускает периодические задачи каждые 10 секунд
- **Background Payment Check Task** (`check_pending_payments`) — проверяет статус pending заказов
- **PaymentStatusService** — централизованная логика обработки статусов платежей
- **Fast/Slow Track Logic** — интеллектуальное планирование проверок

#### Дополнительные поля Order для фоновой проверки

- `payment_started_at` — время перенаправления в платежную систему
- `next_check_at` — время следующей проверки статуса
- `last_check_at` — время последней проверки
- `check_attempts` — количество попыток проверки (лимит: 10)
- `failed_presentation_desc` — описание ошибки для пользователя
...
6. **manual_make** — Оплата прошла, но требуется ручное приготовление (клиент ушел)
7. **successful** — Заказ успешно завершен
8. **failed** — Заказ не может быть завершен (критическая ошибка)
9. **make_failed** — Ошибка при приготовлении напитка

#### Логика Fast/Slow Track

**Fast Track** (≤ 5 минут с момента payment_started_at):
- Проверка каждые 5 секунд
- При успешной оплате: автоматическое приготовление напитка
- Статус: `pending` → `paid` → `make_pending` → `successful`

**Slow Track** (> 5 минут с момента payment_started_at):
- Проверка каждые 60 секунд
- При успешной оплате: статус `manual_make` (требуется ручное приготовление)
- Статус: `pending` → `manual_make`

#### Обработка ошибок

- **Сетевые ошибки**: повторные попытки до лимита (10 попыток)
- **Превышение лимита**: статус `failed` с описанием ошибки в `failed_presentation_desc`
- **Таймаут API**: 3 секунды для всех запросов к платежным системам
- **Протухание заказа**: проверка `expires_at` перед обработкой
...
Webhook'и от платежных систем также используют `PaymentStatusService` для обработки статусов, обеспечивая единообразную логику обработки платежей независимо от источника уведомления (webhook или фоновая проверка).
...
└── payments/
    ...
    └── order_status_page.html     # Unified страница отслеживания статуса заказа (для всех статусов)
```
...
4. Выполняется цепочка валидаций (хеш запроса, существование заказа, статус устройства)
5. Проверяется существование Device и активность Merchant через функции `validate_device()` и `validate_merchant()` из qr_code_service
6. Получается информация о напитке из Tmetr API
7. Создается Order со статусом `created` и временем протухания (expires_at), либо используется существующий валидный заказ
8. Определяется платежный сценарий для устройства (`device.payment_scenario`)
9. **Для Yookassa/TBank**: перенаправление на unified order status page (`/v1/order-status-page?order_id=X`)
10. **Для Custom**: выполняется прямой редирект на внешний URL через `PaymentScenarioService.execute_scenario()`

### 2. Unified Order Status Page
1. Страница загружает информацию о заказе через API (`GET /v1/order-status/<order_id>`)
2. API возвращает полную информацию о заказе, включая `expires_at` и статус-специфичную информацию
3. **Для статуса `created`**:
   - Отображается информация о заказе с кнопкой "Перейти к оплате"
   - Проверяется валидность заказа по времени (expires_at)
   - При нажатии кнопки вызывается `initiate_payment()` через POST-запрос
   - Создается платеж через API платежной системы
   - Статус Order меняется на `pending`
   - Пользователь редиректится на страницу оплаты платежной системы
4. **Для статусов `pending`, `paid`, `make_pending`**:
   - Отображается текущий статус с анимацией загрузки
   - Запускается polling (каждую секунду) для обновления статуса
   - UI обновляется без перезагрузки страницы
5. **Для статуса `not_paid`**:
   - Отображается сообщение об ошибке
   - Показывается кнопка "Повторить оплату"
   - Polling останавливается
6. **Для статусов `successful`, `failed`**:
   - Отображается финальный статус
   - Polling останавливается
...
7. Unified Order Status Page автоматически обнаруживает изменение статуса через polling
...
## Конфигурация типа проверки статуса заказа

Система поддерживает гибкую настройку способа проверки статуса платежей для каждого платежного сценария. Это позволяет оптимизировать работу с различными платежными системами и снизить нагрузку на фоновые задачи.

### Типы проверки статуса (status_check_type)

Система поддерживает три типа проверки статуса платежей:

1. **polling** (по умолчанию) — фоновый опрос платежной системы
   - Celery задача периодически проверяет статус платежа через API платежной системы
   - Используется fast/slow track логика для оптимизации частоты проверок
   - Подходит для платежных систем, которые не поддерживают webhook или имеют ненадежную доставку уведомлений
   - Обеспечивает надежность обработки платежей даже при сбоях webhook'ов

2. **webhook** — получение уведомлений от платежной системы
   - Статус заказа обновляется только при получении webhook от платежной системы
   - Фоновая задача Celery не проверяет такие заказы
   - Снижает нагрузку на систему и API платежной системы
   - Подходит для платежных систем с надежной доставкой webhook'ов

3. **none** — без автоматической проверки
   - Статус заказа не проверяется автоматически
   - Фоновая задача Celery не проверяет такие заказы
   - Используется для специальных сценариев или тестирования

### Настройка типа проверки в MerchantCredentials

Тип проверки статуса настраивается для каждой комбинации (мерчант, платежный сценарий) в модели `MerchantCredentials`:
...
class MerchantCredentials(models.Model):
    ...
    status_check_type = models.CharField(
        ...
        default='polling',
        help_text='Type of payment status check'
    )
```
...
1. Войдите в Django Admin (`/admin/`)
2. Перейдите в раздел "Merchant Credentials"
3. Выберите или создайте запись учетных данных
4. В поле "Status check type" выберите нужный тип:
   - **Polling** — для фоновой проверки (по умолчанию)
   - **Webhook** — если платежная система надежно отправляет webhook'и
   - **None** — для отключения автоматической проверки
5. Сохраните

### Фиксация типа проверки в заказе

При создании платежа система автоматически копирует `status_check_type` из `MerchantCredentials` в заказ (`Order.status_check_type`). Это значение фиксируется и не изменяется в течение жизненного цикла заказа, даже если настройки мерчанта будут обновлены.

**Логика фиксации:**

1. При инициации платежа система получает `MerchantCredentials` для текущего платежного сценария
2. Значение `status_check_type` копируется в `Order.status_check_type`
3. Если учетные данные не найдены, используется значение по умолчанию `'polling'`
4. Значение сохраняется в заказе и больше не изменяется

**Пример в коде:**

```python
# В функции initiate_payment
...
### Влияние на фоновую проверку платежей

Фоновая задача Celery (`check_pending_payments`) обрабатывает только заказы с `status_check_type='polling'`. Заказы с типами `'webhook'` и `'none'` исключаются из фоновой обработки.

**Фильтрация в Celery задаче:**

```python
orders = Order.objects.filter(
    status='pending',
    status_check_type='polling',  # Только polling заказы
...
- Снижение нагрузки на фоновые задачи
- Уменьшение количества запросов к API платежных систем
- Более эффективное использование ресурсов сервера
- Гибкость в выборе способа интеграции с платежными системами

### Рекомендации по выбору типа проверки

**Используйте `polling`:**
- Для платежных систем без поддержки webhook
- Для платежных систем с ненадежной доставкой webhook'ов
- Когда требуется максимальная надежность обработки платежей
- Для тестовых сценариев и разработки

**Используйте `webhook`:**
- Для платежных систем с надежной доставкой webhook'ов (например, Yookassa)
- Когда нужно снизить нагрузку на систему
- Для высоконагруженных сценариев с большим количеством заказов

**Используйте `none`:**
- Для специальных сценариев без автоматической обработки
- Для тестирования и отладки
- Когда обработка платежей выполняется вручную

### Мониторинг и логирование

Все операции, связанные с типом проверки статуса, логируются:

- Установка `status_check_type` при создании платежа
- Фильтрация заказов в фоновой задаче по типу проверки
- Количество обработанных заказов каждого типа

**Пример логов:**

```
[2025-11-23T10:30:00+03:00] INFO [initiate_payment] Set status_check_type='webhook' for Order order-123 from MerchantCredentials (scenario=Yookassa)
[2025-11-23T10:30:10+03:00] INFO [check_pending_payments] Found 5 pending orders to check (status_check_type='polling')
```

### Обратная совместимость

При развертывании фичи все существующие заказы и учетные данные автоматически получают значение `status_check_type='polling'`, что соответствует текущему поведению системы. Никаких ручных действий не требуется.

## Unified Order Status Page (Универсальная страница статуса заказа)

Универсальная страница статуса заказа используется для всех этапов жизненного цикла заказа — от создания до завершения. Пользователь попадает на эту страницу сразу после сканирования QR-кода (для сценариев Yookassa/TBank) или после завершения оплаты. Страница динамически адаптируется к текущему статусу заказа и обновляется в реальном времени без перезагрузки.
...
- Цена в рублях
- Время истечения заказа (`expires_at`) с проверкой валидности
...
- Иконка: 📋
- Описание: "Подтвердите заказ для оплаты"
- Кнопка: "Перейти к оплате" (инициирует платеж через `initiate_payment()`)
- Информация: `Device.client_info`
- Проверка истечения срока: кнопка отключается, если заказ протух
- Polling: не запускается (статический экран)
...
**failed** (Ошибка):
- Иконка: предупреждение (ошибка)
- Описание: "Произошла ошибка при обработке заказа"
- Polling: остановлен
...
- Polling останавливается для статусов: `created`, `successful`, `not_paid`, `failed`
- При каждом polling запросе проверяется валидность заказа по времени (`expires_at`)
- Если заказ протух во время просмотра, показывается предупреждение и polling останавливается
...
- `initiate_payment()` — обрабатывает POST-запрос при нажатии кнопки "Перейти к оплате"

**URL endpoints:**
- `/v1/order-status-page?order_id=<order_id>` — unified страница статуса заказа (для всех статусов)
- `/v1/order-status/<order_id>` — API для получения статуса заказа (используется для polling)
- `/v1/initiate-payment` — инициация платежа для заказов в статусе `created`
...
- Универсальная страница для всех статусов заказа (включая `created`)
- Автоматическое обновление статуса каждую секунду (для не-терминальных статусов)
- Плавные переходы между состояниями
- Поддержка HTML в информационных полях
- Функция инициации платежа для статуса `created`
- Функция повторной оплаты для статуса `not_paid`
- Проверка истечения срока заказа на клиентской стороне
- Централизованные сообщения из `user_messages.py`
- Поддержка accessibility (high contrast mode, reduced motion)

**Поля Device для статус-специфичной информации:**
- `client_info` — информация для статуса created (поддерживает HTML)
...
```json
{
  ...
  "device": {
    ...
    "client_info": "Контакты поддержки: +7 900 123-45-67",
    "client_info_pending": "...",
    "client_info_paid": "...",
...
Система использует структурированное JSON логирование с выводом в stdout/stderr для совместимости с Kubernetes лог-агрегацией.

**Ключевые особенности:**
- Все логи выводятся в stdout (INFO, DEBUG, WARNING) и stderr (ERROR, CRITICAL)
- JSON формат для всех логов
- Централизованное управление уровнем логирования через переменную окружения LOG_LEVEL
- Совместимость с Loki, ELK, CloudWatch и другими системами лог-агрегации

**Конфигурация:**
- Установите переменную окружения LOG_LEVEL для управления детализацией логов
- Уровень по умолчанию: INFO
- Все логгеры (Django, Celery, пользовательские) используют LOG_LEVEL
...
**Без файлового логирования:**
- Логи не записываются в файлы
- Не требуется ротация логов
- Kubernetes автоматически собирает и управляет логами
