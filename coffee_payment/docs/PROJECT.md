...
4. **Для Yookassa/TBank**: отображается экран информации о заказе с деталями напитка, ценой и кнопкой "Перейти к оплате"
5. Пользователь нажимает кнопку, вызывается `initiate_payment()`
6. Создается платеж через API платежной системы, статус меняется на `pending`
7. Пользователь редиректится на страницу оплаты
8. **Для Custom**: прямой редирект на внешний URL (без экрана информации)
