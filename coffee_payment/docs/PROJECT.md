...
4. Пользователь редиректится на страницу оплаты

### 3. Обработка результата платежа (Webhook)
1. YooKassa отправляет webhook при успешном платеже
2. `yookassa_payment_result_webhook()` обновляет статус Order
3. Отправляется команда на кофемашину через Tmetr API
4. Кофемашина начинает приготовление напитка
